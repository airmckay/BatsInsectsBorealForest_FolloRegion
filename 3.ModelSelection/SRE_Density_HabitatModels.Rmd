...
myot <- bats1 %>% filter(manual.id == "MYOT") %>% select(-behavior) %>% droplevels() %>% distinct()
dim(myot)
summary(myot)
#16161    33

plau <- bats1 %>% filter(manual.id == "PAUR") %>% select(-behavior) %>% droplevels() %>% distinct()
dim(plau)
summary(plau)
#9407   33
...
pred.p <- ggplot(newdata.p, aes(x = s.PC1, y = predicted,
  ...
  # Change legend title
  labs(x = "Habitat openness", y = "Bat passes per night", 
       title = expression(paste(italic("Plecotus auritus "), "activity"))) +
  scale_color_manual(name = "Habitat", values = plot.cols) +
  scale_fill_manual(name = "Habitat", values = plot.cols) 
pred.p
...
ggsave(path = output_today, filename = "SREmodelPreds.tiff", width = 10, height = 8, device='tiff', dpi=300) 
