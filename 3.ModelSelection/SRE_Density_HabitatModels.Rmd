...
newer.m.ob <- newdata.m %>% 
  dplyr::filter(PlotType == "Open" & s.PC1 < -0.6997) %>% droplevels()
dim(newer.m.ob)
 # 7128      12

test <- newdata.m %>% dplyr::filter(PlotType == "Interior") 
range(test$s.PC1)

newer.m.ib <- newdata.m %>% 
  dplyr::filter(PlotType == "Interior" & s.PC1 > 0.60446) %>% 
  droplevels()
dim(newer.m.ib)
 # 4536    12

# -2.93  1.95
newer.m.cb <- newdata.m %>% 
  dplyr::filter(PlotType == "Canopy") %>% 
  droplevels() %>% 
  dplyr::mutate(PlotType1 = 
                   factor(case_when(
  s.PC1 < -2.73814 ~ "Canopy1", 
  s.PC1 > 0.88423 ~ "Canopy2",
  .default = "Nope"))) %>% 
  dplyr::filter(PlotType1 != "Nope") %>% 
  dplyr::select(-PlotType) %>% 
  rename(PlotType = PlotType1) %>% 
  droplevels()
summary(newer.m.cb)
dim(newer.m.cb)
# 4536       12
...
temp1 <- full_join(newer.m.ob, newer.m.ib)
   # 7128 + 4536 = 11664 # good 
temp2 <- full_join(temp1, newer.m.cb)
   # 11664 + 4536  = 16200

newer.m <- temp2
summary(newer.m)

pred.m.gray <- ggplot(newer.m, 
                     aes(x = s.PC1, y = predicted,
                     group = PlotType)) +
  # Add average line for each combination of PlotType and Season
  stat_summary(fun = "mean",  
               geom = "line", linewidth = 1, 
...
plOB <- pldf %>% filter(PlotType == "Open") %>% droplevels
#    Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# -0.6997  0.3574  0.9611  0.7640  1.1265  1.9511 
## Range = -0.6997 - 1.9511

plIB <- pldf %>% filter(PlotType == "Interior") %>% droplevels
summary(plIB$s.PC1)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -2.92499 -0.61295 -0.49881 -0.44561  0.05632  0.60446 

plCB <- pldf %>% filter(PlotType == "Canopy") %>% droplevels
summary(plCB$s.PC1)
#     Min.  1st Qu.   Median     Mean  3rd Qu.     Max. 
# -2.73814 -0.75569  0.09113 -0.31163  0.42900  0.88423 
...
newer.p.ob <- newdata.p %>% 
  dplyr::filter(PlotType == "Open" & s.PC1 < -0.6997) %>% droplevels()
dim(newer.p.ob)
 # 7128      12

test <- newdata.p %>% dplyr::filter(PlotType == "Interior") 
range(test$s.PC1)

newer.p.ib <- newdata.p %>% 
  dplyr::filter(PlotType == "Interior" & s.PC1 > 0.60446) %>% 
  droplevels()
dim(newer.m.ib)
 # 4536    12
...
temp1 <- full_join(newer.p.ob, newer.p.ib)
   # 7128 + 4536 = 11664 # good 
temp2 <- full_join(temp1, newer.p.cb)
   # 11664 + 4536  = 16200

newer.p <- temp2
summary(newer.p)

pred.p.gray <- ggplot(newer.p, 
                     aes(x = s.PC1, y = predicted,
                     group = PlotType)) +
  # Add average line for each combination of PlotType and Season
  stat_summary(fun = "mean",  
...
cowplot::plot_grid(pred.m.gray, pred.p.gray)
ggsave(path = output_today, filename = "test.gray.tiff", width = 12, height = 8, device='tiff', dpi=300) 

cowplot::plot_grid(pred.m, pred.p)
ggsave(path = output_today, filename = "test.tiff", width = 12, height = 8, device='tiff', dpi=300) 


## Did not work the way I had hoped it would 
```
...
general<- ggplot(bats1 %>%
                   filter(batpass>0) %>% 
                  filter(guild == "SRE") %>% droplevels()) + 
  geom_area(aes(x=jnight, y = day_length), fill = "#f0e2a8") +
  geom_smooth(aes(x  = jnight, y = batpass), 
