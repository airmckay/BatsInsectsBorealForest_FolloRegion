...
surveyeffort <- cowplot::plot_grid(S1.21, S1.22, nrow = 2, labels = c("a.", "b."))

surveyeffort

# ggsave(path = output_today, filename = "SurveyEffort.tiff", width = 8, height = 10, device='tiff', dpi=300) 
...
rbats$auto.guild <- factor(rbats$auto.guild, levels=c("SRE", "MRE", "LRE", "NoID")) 
...
# Exploratory: 
# First check variation across sites 
# ggplot(data = bats1 %>% filter(batpass > 0)) + 
#   aes(x = PlotType, fill = manual.id, by = PlotType) +
#   geom_bar(position = "fill", stat = "prop") + 
#   scale_fill_manual(values = mid.cols) + 
#   theme_minimal() + 
#   facet_wrap(~SiteYear) 

# Now for only plot types, all sites combined, separate years 
taxaplot <- ggplot(data = bats1 %>% 
                     filter(batpass > 0)) + 
  aes(x = PlotType, fill = taxa, by = PlotType) +
  geom_bar(position = "fill", stat = "prop") + 
  scale_fill_manual(values = mid.cols, name="",
                    labels = levels(bats1$taxa), 
                    expression(italic())) + 
  theme_minimal() + 
  facet_wrap(~year) +
...
guildplot <- ggplot(data = bats1 %>% filter(batpass >0) %>% droplevels()) +
  aes(x = behavior, fill = PlotType, by = behavior) +
  geom_bar(position = "fill", stat = "prop") +
...
figs3 <- cowplot::plot_grid(taxaplot, guildplot, nrow = 2, labels = c("a.", "b.")) 
figs3

ggsave(path = output_today, filename = "behavior_guild_boxplots.tiff", width = 8, height = 8, device='tiff', dpi=300) 

### Box plots 

## All SRE activity across plot types 
S<- ggplot(data = bats1 %>%
         filter(batpass > 0) %>% 
         filter(guild == "SRE") %>% 
         droplevels()) + 
  aes(x = PlotType, fill = PlotType, y = batpass30) +
  geom_boxplot() + 
  scale_fill_manual(values = plot.cols) + 
  theme_minimal() +
  ggtitle("SRE")
...
# Myotis feeding activity 
Mfeed <- ggplot(data = bats1 %>%
         filter(batpass > 0) %>% 
         filter(manual.id == "MYOT") %>% 
         filter(behavior == "Feeding") %>% 
         droplevels()) + 
