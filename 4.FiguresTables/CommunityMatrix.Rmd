...
sites
ggsave(path = output_today, filename = "CommunitySites.tiff", width = 20, height = 10, device='tiff', dpi=300) 
# 2022 = dark
# 2021 = light
...
bats1$taxa <- gsub("MYOT", "Myotis", bats1$taxa)
bats1$taxa <- gsub("PAUR", "P.auritus", bats1$taxa)
bats1$taxa <- factor(bats1$taxa)

behavplot <- ggplot(data = bats1 %>%
                     filter(guild == "SRE") %>% 
                     filter(batpass > 0) %>% 
                      droplevels()) + 
...
behavplot
ggsave(path = output_today, filename = "BehaviorFigure.tiff", width = 10, height = 12, device='tiff', dpi=300) 
